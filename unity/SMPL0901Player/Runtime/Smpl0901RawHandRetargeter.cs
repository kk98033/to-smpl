...
{
    public class Smpl0901RawHandRetargeter : MonoBehaviour
    {
        ...
        public void Initialize(Transform newCharacterRoot)
        {
            ...
            Debug.Log($"[SMPL0901 Hands] mapped left={LeftMappedBones}, right={RightMappedBones} finger bones (ROM limits={(enableJointLimits ? "ON" : "OFF")}).");
        }
            ...
            bool isLeft)
        {
            if (wrist == null)
            {
                Debug.LogWarning($"[SMPL0901 Hands] Could not find {prefix} wrist.");
                return;
            }
